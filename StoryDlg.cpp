...
BOOL CStoryDlg::OnInitDialog()
{
	...
	if (Act == 1)
	{
		GetDlgItem(IDOK)->SetWindowText(_TEXT("继续游戏"));
...
void CStoryDlg::ProcessHelpFile()
{
	...
	GetDlgItem(IDC_INIT_MSG)->SetWindowText(_TEXT("初始化帮助信息...."));
	...
	GetDlgItem(IDC_INIT_MSG)->SetWindowText(_TEXT("游戏初始化完毕,准备进入北京..."));
}
...
void CStoryDlg::InitUser()
{
	int i, j, k;
	CString msg;
	GetDlgItem(IDC_INIT_MSG)->SetWindowText(_TEXT("初始化黑市信息...."));
	for (i = 0; i < 10; i++)
		for (j = 0; j < 10; j++)
			for (k = 0; k < 1000; k++) {
				//msg.Format(_TEXT("%d"),k);
   //GetDlgItem(IDC_INIT_MSG)->SetWindowText(msg);
			}
	GetDlgItem(IDC_INIT_MSG)->SetWindowText(_TEXT("初始化二环路信息...."));
	AfxMessageBox(_TEXT("ok"));
}
